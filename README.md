...
Next, run the code to generate the shell script
``` shell
cd run
python gen_sh.py 
```
...
We should run the script one by one, and wait for it for a day or more.
Run the script, to let a python script to help us to do this job.
``` shell
python Thread_Mgr.py
```
This script will generate multiple threads to run the scripts in forder */sh/*, it can save our time. 
<font color= red>But it should be noted that if the computer performance is not high enough and the number of programs running at the same time is too large, the programs may compete with each other for computer resources, and the encoding time will be inaccurate.</font>
